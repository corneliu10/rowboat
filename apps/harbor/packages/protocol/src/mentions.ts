...
import { MENTION_HANDLE } from './brand.js';
 ...
 * `[@…](#member:…)` / `[@here](#here)` / `[@spinball](#spinball)` address;
 * `[#…](#space:…)` refers.
 */
// Built from MENTION_HANDLE so the wire handle lives in one place (brand.ts).
// Note: the global flag is required — mapMentionTokens relies on replace-all.
export const MENTION_TOKEN_RE = new RegExp(
  `\\[@([^\\]\\n]*)\\]\\(#(?:member:([^)\\s]+)|(here)|(${MENTION_HANDLE}))\\)|\\[#([^\\]\\n]*)\\]\\(#space:([^)\\s]+)\\)`,
  'g',
);
...
function refOf(label: string, id: string | undefined, here: string | undefined, spaceLabel: string | undefined, spaceId: string | undefined): MentionRef {
  ...
  return here !== undefined ? { kind: 'here' } : { kind: 'spinball' };
}
...
export function mapMentionTokens(body: string, fn: (ref: MentionRef, raw: string) => string): string {
  return body
    ...
    .map((part, i) => {
      ...
      // Reset lastIndex: MENTION_TOKEN_RE is global and shared — replace
      // relies on it starting at 0 for each part.
      MENTION_TOKEN_RE.lastIndex = 0;
      return part.replace(MENTION_TOKEN_RE, (raw: string, label: string, id?: string, here?: string, _spinball?: string, spaceLabel?: string, spaceId?: string) =>
...
export function parseMentions(body: string): MentionStamps {
  ...
  mapMentionTokens(body, (ref, raw) => {
    ...
    else if (ref.kind === 'spinball') spinball = true;
    return raw;
  });
  return { members, here, spinball };
}

/** True when the body deliberately addresses @spinball (a token, never the word). */
// Kept name addressesRowboat: the stamp field it reads ripples through protocol types; value is now spinball.
export function addressesRowboat(body: string): boolean {
  return parseMentions(body).spinball;
}
...
export const MENTION_GRAMMAR =
  ...
  `\`[@here](#here)\` addresses everyone in the space and \`[@${MENTION_HANDLE}](#${MENTION_HANDLE})\` addresses the ` +
