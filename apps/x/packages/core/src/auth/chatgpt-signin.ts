      ...
      try {
        boundServer = await openLoopback(CHATGPT_CALLBACK_PORT, onCallback, {
          ...
          validateCallback: (url) => {
            if (settled) {
              return 'This sign-in attempt is no longer active. Close this tab and try again from Spinrun.';
            }
            if (url.searchParams.get('state') !== state) {
              return 'This sign-in link has expired. Close this tab and try again from Spinrun.';
            }
            return null;
          },
        });
